...
int _EpsonThermalFull::Decode(QByteArray *Data, TicketCanvass *ticketCanvass, int *Offset) {
    int x_iter = 0;
    int y_iter = 0;
    int x_pos = 0;
    int y_pos = 0;
    ...
    uint8_t mode, x_low, x_high, y_low, y_high, x_scale, y_scale, pixel;
    int pixcount=0;
    ptr = (*Offset);
    while(ptr < Data->size()){
        if( (Data->at(ptr) == 0x1D) && (Data->at(ptr+1) == 0x38) && (Data->at(ptr+2) == 0x4C) &&  // GS 8 L
            ...
            x_scale = uint8_t(Data->at(ptr+10));
            y_scale = uint8_t(Data->at(ptr+11));
            ...
            for(y_iter = 0; y_iter < y_size; y_iter++) {
                ...
                for(x_iter = 0; x_iter < x_size; x_iter += 8) {
                    ...
                    ptr++;
                    length--;
                    if(x_scale == 0x01 && y_scale == 0x01) {
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos,pixel,false);
                    }
                    else if(x_scale == 0x02 && y_scale == 0x01) {
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos,pixel,true);
                        ticketCanvass->DrawByteHorizontal(x_pos+1,y_pos,pixel,true);
                    }
                    else if(x_scale == 0x01 && y_scale == 0x02) {
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos,pixel,false);
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos+1,pixel,false);
                    }
                    else if(x_scale == 0x02 && y_scale == 0x02) {
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos,pixel,true);
                        ticketCanvass->DrawByteHorizontal(x_pos+1,y_pos,pixel,true);
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos+1,pixel,true);
                        ticketCanvass->DrawByteHorizontal(x_pos+1,y_pos+1,pixel,true);
                    }
                    pixcount += 8;
                    if(x_scale == 0x01) {
                        x_pos += 8;
                    }
                    else if(x_scale == 0x02) {
                        x_pos += 16;
                    }
                }
                if(y_scale == 0x01) {
            ...
            for(y_iter = 0; y_iter < y_size; y_iter++) {
                ...
                for(x_iter = 0; x_iter < x_size; x_iter++) {
                    ...
                    else if(mode == 0x02 || mode == 0x32) {
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos,pixel,false);
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos+1,pixel,false);
                    }
                    else if(mode == 0x03 || mode == 0x33) {
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos,pixel,true);
                        ticketCanvass->DrawByteHorizontal(x_pos+1,y_pos,pixel,true);
                        ticketCanvass->DrawByteHorizontal(x_pos,y_pos+1,pixel,true);
                        ticketCanvass->DrawByteHorizontal(x_pos+1,y_pos+1,pixel,true);
                    ...
                    pixcount += 8;
                    if(mode == 0x00 || mode == 0x30 || mode == 0x02 || mode == 0x32) {
                        x_pos += 8;
                    }
                    else if(mode == 0x01 || mode == 0x31 || mode == 0x03 || mode == 0x33) {
                        x_pos += 16;
                    }
                }
                if(mode == 0x00 || mode == 0x30 || mode == 0x01 || mode == 0x31) {
                    y_pos += 1;
                }
                else if(mode == 0x02 || mode == 0x32 || mode == 0x03 || mode == 0x33) {
                    y_pos += 2;
                }
            }
            x_offset = 0;
