...
HasilOlah* DecodeTask::decodeData(int printerSelection){
    ...
    if(pixcount) {
        ...
        int blurValue = (dataRaw.profilePe / 10) % 10;
        if(blurValue == 0) {
            ticketCanvass->GetCanvass(&tmpHasilOlah->imageHasil, &tmpHasilOlah->teksHasil, pixelScale);
        }
        else {
            ticketCanvass->GetCanvass(&tmpHasilOlah->imageHasil, &tmpHasilOlah->teksHasil, pixelScale, blurValue);
        }
    }
    else {
        tmpHasilOlah->indexSumber = dataRaw.indexNum;
